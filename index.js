 ...
 *
 * Modeled on the packaged dsh skill providers and the creght-dev/skills
 * reference plugin.
 * @module dsh-waimai-discount
 */
...
const PROVIDER_NAME = 'waimai'

const SKILLS_ROOT = fileURLToPath(new URL('./skills/', import.meta.url))

export const name = 'dsh-waimai-discount'
...
export function apply(ctx) {
  ...
  if (loadSkillsSync().length === 0) {
    throw new Error(`dsh-waimai-discount: no SKILL.md bundles found under ${SKILLS_ROOT}`)
  }
...
/**
 * Read every `skills/<name>/SKILL.md` bundle from disk on each discovery, so an
 * edited skill body is picked up without restarting the harness.
...
function parseSkill(raw, directory, skillFile) {
  ...
  if (parsed === undefined) {
    throw new Error(`dsh-waimai-discount: ${skillFile} is missing YAML frontmatter`)
  }
  const skillName = stringField(parsed.data, 'name')
  const description = stringField(parsed.data, 'description')
  if (skillName === undefined || description === undefined) {
    throw new Error(`dsh-waimai-discount: ${skillFile} frontmatter requires name and description`)
  }
  if (!SKILL_NAME.test(skillName)) {
    throw new Error(`dsh-waimai-discount: invalid skill name "${skillName}"`)
  }
  return {
    name: skillName,
    description,
...
function sortSkills(skills) {
  ...
  for (const skill of skills) {
    if (names.has(skill.name)) {
      throw new Error(`dsh-waimai-discount: duplicate skill name "${skill.name}"`)
    }
